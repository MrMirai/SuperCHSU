...
{
    private class MyDisposable<TObj, TProp> : IDisposable where TObj : INotifyPropertyChanged
    {
        public MyDisposable(Action handler, TObj source, string propertyName)
        {
            Source = source;
        ...
        public MyDisposable(Action<TProp> handler, TObj source, Func<TObj, TProp> selector, string propertyName)
        {
            Source = source;
    ...
    public static IDisposable WhenPropertyChanged<TObject, TProperty>(this TObject sender,
        Expression<Func<TObject, TProperty>> propertyAccessor, Action<TProperty> action) where TObject : INotifyPropertyChanged
    {
    ...
    /// <returns><see cref="IDisposable"/></returns> 
    public static IDisposable WhenPropertyChanged<TObject, TProperty>(this TObject sender,
        Expression<Func<TObject, TProperty>> propertyAccessor, Action action) where TObject : INotifyPropertyChanged
    {
        var member = propertyAccessor.Body as MemberExpression;
        var propInfo = member?.Member as PropertyInfo;
